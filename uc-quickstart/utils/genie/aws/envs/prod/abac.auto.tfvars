...
genie_benchmarks = [
  {
    ...
    sql = "SELECT SUM(amount) as total_amount FROM prod_fin.finance.transactions WHERE status = 'COMPLETED'"
  },
  {
    question = "How many patient encounters occurred last month?"
    sql = "SELECT COUNT(*) FROM prod_clinical.clinical.encounters WHERE encounter_date >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL 1 MONTH) AND encounter_date < DATE_TRUNC('month', CURRENT_DATE)"
  },
  {
    question = "What is the average credit limit for active credit cards?"
    sql = "SELECT AVG(credit_limit) as avg_credit_limit FROM prod_fin.finance.credit_cards WHERE status = 'ACTIVE'"
  },
  {
    question = "How many verified customers are in the private banking tier?"
    sql = "SELECT COUNT(*) FROM prod_fin.finance.customers WHERE kyc_status = 'VERIFIED' AND account_tier = 'PRIVATE_BANKING'"
  },
  {
    question = "What is the total billed amount for emergency encounters this year?"
    sql = "SELECT SUM(billed_amount) FROM prod_clinical.clinical.encounters WHERE encounter_type = 'EMERGENCY' AND YEAR(encounter_date) = YEAR(CURRENT_DATE)"
  },
]
...
genie_join_specs = [
  {
    left_table = "prod_fin.finance.transactions"
    left_alias = "transactions"
    right_table = "prod_fin.finance.customers"
  ...
  {
    left_table = "prod_fin.finance.credit_cards"
  ...
  {
    left_table = "prod_clinical.clinical.encounters"
    left_alias = "encounters"
    right_table = "prod_clinical.clinical.patients"
